...
> A dynamic table _T_ is an array with changeable length. When the array is full, we increase its length; when the array is "Too empty", we reduce its length.
...
- _T.table_ : the array itself
- _T.size_ : length of the array
- _T.num_ : number of elements in _T_
...
If there are _n_ insertions, the total cost of _n_ operations is below.
-Note: do not consider the following. - There is only one table expansion, and $$T.size = m >> n$$. In this case, there are not enough operations to be an amortized analysis. - Thus, assume that during all _n_ insertions, $$T.size = m$$ can be at most $$O(n)$$.
With the note above, can see that _n_ operations have a total cost $$O(n^2)$$, but this upper bound is not tight.

### Total cost of _n_ insertions

There are two parts. One is i) **_n_ simple insertions** and ii) **some table expansions**.

#### i) _n_ simple insertions

&emsp;It needs _n_ times.
